...
public class StatStorageManager{
	...
	public HashMap<Integer, Double> getCpuUsagePerWorker(String component, String host, Integer port, Integer timestamp, Integer windowSize){
		int oldestTimestamp =  timestamp - windowSize;
		HashMap<Integer, Double> records = new HashMap<>();
		try {
			String query  = "SELECT timestamp, " + COL_CPU_USAGE + " FROM " + TABLE_BOLT 
			...
			while(results.next()){
				...
				Double usage = 0.0;
				if(records.containsKey(readTimestamp)){
					usage = records.get(readTimestamp);
				}
				Double cpuUsage = new BigDecimal(results.getDouble(COL_CPU_USAGE)).setScale(3, BigDecimal.ROUND_HALF_UP).doubleValue();
				usage += cpuUsage;
				records.put(readTimestamp, usage);
			}
			results.close();
		} catch (SQLException e) {
			logger.severe("Unable to retrieve the average cpu usage of the component " + component + " on worker " + host + "@" + port + " because of " + e);
		}
		return records;
	}
